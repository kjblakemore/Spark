...
parsedSamplePoints = parsePoint(rawData.take(1)[0])
...
onlyLabels = parsedDataInit.map(lambda point: point.label)
...
# TODO: Replace <FILL IN> with appropriate code
parsedData = parsedDataInit.map(lambda point: LabeledPoint(point.label-1922, point.features))
...
# TODO: Replace <FILL IN> with appropriate code
averageTrainYear = (parsedTrainData.map(lambda point: point.label)
                   .mean())
...
def calcRMSE(labelsAndPreds):
    ...
    """
    num = labelsAndPreds.count()
    rmse = np.sqrt(labelsAndPreds.map(lambda (x, y): squaredError(x, y))
                   .mean())
...
# TODO: Replace <FILL IN> with appropriate code
labelsAndPredsTrain = parsedTrainData.map(lambda point: (point.label, averageTrainYear))  # averageTrainYear from (2a)
rmseTrainBase = calcRMSE(labelsAndPredsTrain)

labelsAndPredsVal = parsedValData.map(lambda point: (point.label, averageTrainYear))
rmseValBase = calcRMSE(labelsAndPredsVal)

labelsAndPredsTest = parsedTestData.map(lambda point: (point.label, averageTrainYear))
...
def linregGradientDescent(trainData, numIters):
    ...
    for i in range(numIters):
        ...
        gradient = trainData.map(lambda lp: gradientSummand(w, lp)).reduce(lambda gs1, gs2: gs1 + gs2)
...
# TODO: Replace <FILL IN> with appropriate code
firstModel = LinearRegressionWithSGD.train(parsedTrainData, iterations=numIters, step = alpha, regParam=reg, regType=regType, intercept=useIntercept)
...
def twoWayInteractions(lp):
    ...
    """
    label = lp.label
    features = np.hstack((lp.features, [f1*f2 for (f1, f2) in itertools.product(lp.features, repeat = 2)]))
