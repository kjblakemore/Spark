...
dailyHosts = dayGroupedHosts.mapValues(lambda hosts: len(hosts)).cache()
...
daysWithHosts = [x[0] for x in dailyHosts.takeOrdered(30)]
hosts = [x[1] for x in dailyHosts.takeOrdered(30)]
...
groupedByDay = dayAndHostTuple.groupByKey().mapValues(lambda hosts: len(hosts))

sortedByDay = groupedByDay.join(dailyHosts)
               
avgDailyReqPerHost = (sortedByDay.map(lambda (day, (count, uniquehosts)): (day, count/uniquehosts))).cache()

avgDailyReqPerHostList = sorted(avgDailyReqPerHost.collect())
...
badEndpointsCountPairTuple = badRecords.map(lambda log404: (log404.endpoint, 1))
...
errHostsCountPairTuple = badRecords.map(lambda log404: (log404.host, 1))

errHostsSum = errHostsCountPairTuple.reduceByKey(lambda a, b: a+b)

errHostsTop25 = errHostsSum.takeOrdered(25, lambda (host, count): (-1 * host, -1 * count))
...
errDateCountPairTuple = badRecords.map(lambda log404: (log404.date_time.day, 1))
...
daysWithErrors404 = [x[0] for x in errByDate]
errors404ByDay = [x[1] for x in errByDate]
...
hourCountPairTuple = badRecords.map(lambda log404: (log404.date_time.hour, 1))
...
hoursWithErrors404 = [x[0] for x in errHourList]
errors404ByHours = [x[1] for x in errHourList]
