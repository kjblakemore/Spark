...
def getCountsAndAverages(IDandRatingsTuple):
    ...
    countsAndAverages = (movieID, (len(ratings), sum(ratings)/float(len(ratings))))
    return countsAndAverages
...
movieIDsWithRatingsRDD = (ratingsRDD
                          .map(lambda (UseID, MovieID, Rating): (MovieID, Rating))
                          .groupByKey())
...
# #### For movie recommendations, we start with a matrix whose entries are movie ratings by users (shown in red in the diagram below).  Each column represents a movie (shown in red and each row represents a particular user (shown in blue).
...
def computeError(predictedRDD, actualRDD):
    ...
    squaredErrorsRDD = (predictedReformattedRDD
                        ...
                        .map(lambda (ID, (predRating, actRating)): pow(predRating-actRating, 2)))

    # Compute the total squared error - do not use collect()
    totalError = squaredErrorsRDD.reduce(lambda error1, error2: error1 + error2)
...
trainingAvgRating = (trainingRDD
                     ...
                     .reduce(lambda rating1, rating2: rating1 + rating2))/trainingRDD.count()
...
myRatingsRDD = sc.parallelize(myRatedMovies).cache()
...
myUnratedMoviesRDD = (moviesRDD.filter(lambda (movieID, title): movieID not in [x[1] for x in myRatedMovies])
...
print predictedRatingsRDD.take(3)
...
movieCountsRDD = movieIDsWithAvgRatingsRDD.map(lambda (movieID, (count, rating)): (movieID, count))
...
ratingsWithNamesRDD = (predictedWithCountsRDD
                       .filter(lambda (movieID, (rating, count)): count > 75)
                       .join(moviesRDD)
                       .map(lambda (movieID, ((rating, count), title)): (rating, title)))
