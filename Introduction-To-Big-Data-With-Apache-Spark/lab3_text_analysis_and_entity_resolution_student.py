...
split_regex = r'\W+'                                          # Matches any non-alphanumeric character, including '_'
...
def tokenize(string):
    ...
    # First, generate a list of all words in the string
    list = filter(None, re.split(split_regex, string.lower()))
    
    # Next, subtract the stop words
...
# TODO: Replace <FILL IN> with appropriate code
# The RDD value format is 'title "description"'
...
def totalTokenCount(vendorRDD):
    """ total number of tokens in RDD
    Args:
        vendorRDD (RDD of (recordId, tokens)): Pair tuple of record ID to Tokens
    Returns:
        totalTokens: sum of tokens from all records.
    """
    return (vendorRDD
            .map(lambda (recordId, tokens): tokens)
            .map(len)
            .reduce(lambda a, b: a + b))

totalTokens = totalTokenCount(amazonRecToToken) + totalTokenCount(googleRecToToken)
...
def tf(tokens):
    ...
    """
    count = len(tokens)              # number of tokens
    dictionary = {}
    
    # First build dictionary of tokens with corresponding occurence counts
    for word in tokens:
    ...
    # Divide each occurrence count by the total numbertokens
    for word in dictionary:
        dictionary[word] = float(dictionary[word])/count
...
# TODO: Replace <FILL IN> with appropriate code
corpusRDD = (amazonRecToToken
    .union(googleRecToToken)
    .cache())
...
def idfs(corpus):
    """ Compute IDF
    Args:
        corpus (RDD): input corpus, with each record consisting of an id and list of tokens
    Returns:
        RDD: a RDD of (token, IDF value)
    """
    N = amazonRecToToken.count() + googleRecToToken.count()
    
    uniqueTokens = (corpus
                    .map(lambda (id, tokens): tokens)
                    .flatMap(lambda tokens: list(set(tokens))))
...
def tfidf(tokens, idfs):
    ...
    """
    tfs = tf(tokens)     # A dictionary of tokens and corresponding term frequencies
    tfIdfDict = {word:tfs[word] * idfs[word] for word in tfs}
    return tfIdfDict
...
def dotprod(a, b):
    ...
    """
    return sum([a[word]*b[word] for word in a if word in b])
...
def norm(a):
    ...
    """
    return sqrt(sum([a[word]**2 for word in a]))
...
def computeSimilarityBroadcast(record):
    ...
    googleValue = googleRec[1]
    amazonValue = amazonRec[1]
    cs = cosineSimilarity(amazonValue, googleValue, idfsSmallBroadcast.value)
    return (googleURL, amazonID, cs)
...
def parse_goldfile_line(goldfile_line):
    ...
    elif match.group(1) == '"idAmazon"':    # Skip header line
...
# TODO: Replace <FILL IN> with appropriate code
sims = similaritiesBroadcast.map(lambda line: (line[1] + ' ' + line[0], line[2]))
...
avgSimDups = (trueDupsRDD
              .map(lambda (key, (cv, match)): cv)
              .reduce(lambda a, b: a + b)) / trueDupsCount

nonDupsRDD = (sims
                .fullOuterJoin(goldStandard)
                .filter(lambda (key, (cv, match)): match == None))

nonDupsCount = nonDupsRDD.count()

avgSimNon = (nonDupsRDD
                .map(lambda (key, (cv, match)): cv)
                .reduce(lambda a, b: a + b)) / nonDupsCount
...
# TODO: Replace <FILL IN> with appropriate code
amazonFullRecToToken = (amazon
                        .mapValues(tokenize)
                        .cache())
googleFullRecToToken = (google
                        .mapValues(tokenize)
                        .cache())
...
# Pre-compute TF-IDF weights.  Build mappings from record ID to weight vector.
amazonWeightsRDD = amazonFullRecToToken.mapValues(lambda tokens: tfidf(tokens, idfsFullWeights))
googleWeightsRDD = googleFullRecToToken.mapValues(lambda tokens: tfidf(tokens, idfsFullWeights))
...
# * #### Use your invert function to convert the full Amazon and Google TF-IDF weighted token vector datasets into two RDDs where each element is a pair of token and ID/URL that contains that token. These are inverted indicies.
...
# TODO: Replace <FILL IN> with appropriate code
# The weights are tfidf values
amazonWeightsBroadcast = amazonWeightsRDD.collectAsMap()
googleWeightsBroadcast = googleWeightsRDD.collectAsMap()

def fastCosineSimilarity(record):
    """ Compute Cosine Similarity using Broadcast variables
    Args:
        record: ((ID, URL), token list)
    Returns:
    ...
    s = sum([amazonWeightsBroadcast[amazonRec][token] * googleWeightsBroadcast[googleRec][token] for token in tokens])
    value = s / amazonNorms[amazonRec] / googleNorms[googleRec]
